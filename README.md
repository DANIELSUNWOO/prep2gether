...
- ## 발표자료 [팀 발표 PPT (PDF)](docs/presentation.pdf) — 멋쟁이사자처럼 백엔드 24기 1차 팀 프로젝트 발표 자료
...
개인 인프라로 이관한 뒤 운영 중인 코드를 처음부터 다시 훑어볼 계기가 있었습니다. 그 과정에서 발견하고 고친 것들입니다.

**결제 없이 구독이 되는 API가 남아있었습니다.** PortOne 연동 이전, 프론트가 결제 성공 여부를 스스로 판단해 호출하던 `POST /api/subscriptions`가 연동 후에도 지워지지 않고 남아 있었습니다. `SecurityConfig`가 `anyRequest().authenticated()`라 로그인만 하면 누구나 이 엔드포인트를 호출해 결제 없이 프리미엄이 될 수 있는 구조였습니다. 결제 검증을 6단계 재조회로 아무리 단단히 만들어도, 그 검증 자체를 우회하는 경로가 남아 있으면 의미가 없다는 걸 다시 확인하고 컨트롤러와 서비스에서 해당 메서드를 제거했습니다.

**카카오 REST API 키가 프론트 번들에 노출되어 있었습니다.** 브라우저가 직접 `kauth.kakao.com`에 토큰을 요청하는 구조라 Vite 빌드 시 REST API 키가 번들에 평문으로 들어갔습니다. 이 때문에 클라이언트 시크릿을 켜면 로그인 자체가 깨져서, 보안 기능을 꺼둔 채로 운영되고 있었습니다. 토큰 교환을 백엔드로 옮기고(`POST /api/auth/kakao/callback`) 프론트는 인가 코드만 서버에 넘기도록 바꾼 뒤, 키와 시크릿을 재발급했습니다.

**PortOne 웹훅 시크릿이 CI 설정 파일에 실제 값으로 커밋되어 있었습니다.** 테스트용 더미 값들 사이에 이 값만 실제 형식 그대로 박혀 있었습니다. PortOne 콘솔에서 시크릿을 재발급해 기존 값을 무효화하고, 워크플로 파일은 다른 항목들과 동일한 더미 값으로 교체했습니다.

세 건 모두 설계 자체보다 "설계를 우회하는 경로"에서 나온 문제였습니다. 이 경험을 계기로, 인가 규칙을 코드 리뷰가 아니라 테스트로 고정하는 작업을 다음 과제로 잡았습니다(아래 "남은 과제" 참고).
...
인가 규칙(`SecurityConfig`, `JwtAuthenticationFilter`)을 검증하는 테스트가 아직 없어, 위에서 고친 것과 같은 종류의 문제가 다시 생겨도 CI가 잡아내지 못합니다. `@WebMvcTest` 기반 인가 테스트 추가가 다음 순서입니다.
